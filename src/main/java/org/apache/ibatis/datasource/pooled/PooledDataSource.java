...
public class PooledDataSource implements DataSource {
  ...
  /**
   * 获取连接消费时间过长的时候，连接池会打印状态日志并尝试重新获取一个新的连接
   */
  ...
  public PooledDataSource(ClassLoader driverClassLoader, String driver, String url, Properties driverProperties) {
    // 这里复用 UnpooledDataSource 创建连接的逻辑
    ...
    while (conn == null) {
      synchronized (state) {
        ...
        if (!state.idleConnections.isEmpty()) {
          ...
          // 激活的了连接数小于 poolMaximumActiveConnections（最大可活跃连接数量）
          if (state.activeConnections.size() < poolMaximumActiveConnections) {
            ...
            if (longestCheckoutTime > poolMaximumCheckoutTime) {
              ...
              // 如果当前连接是非自动提交的，需要进行回滚，即将执行中的事物，全部回滚
              ...
              // Must wait
              try {
                // 对等待的连接进行统计，通过countWait标记，只记录一次
                if (!countedWait) {
                ...
                // 等待，知道超时，或 pingConnection方法中癸亥连接时的唤醒
